...
@injectable()
class FollowUserService implements IService {
    constructor(@inject(Repository.UserRepository) private readonly _userRepository: IUserRepository) { }

    public async execute(data: { userId: string, followUserId: string }) {
        const user = await this._userRepository.findById(data.userId);
        const followUser = await this._userRepository.findById(data.followUserId);
        ...
        if ((await this._userRepository.hasRelation(userFollow))) throw new AppError('Usuario já seguido!');

        await this._userRepository.saveUserFollow(userFollow);
    }
}
